...
if [ -z "$ETCD_01_PRIVATE_IP" ] || [ -z "$ETCD_02_PRIVATE_IP" ] || [ -z "$ETCD_03_PRIVATE_IP" ] ; then
  echo -e "\033[0;31mCan't create k8s droplets. Please provide the private IP addresses of droplet etcd-01, etcd-02 and etcd-03 using the \$ETCD_01_PRIVATE_IP, \$ETCD_02_PRIVATE_IP, \$ETCD_03_PRIVATE_IP variables, respectively.\033[0m"
  exit 1
fi
...
echo -e "\033[0;32mCreating droplets k8s-master...\033[0m"
...
doctl compute droplet tag k8s-master --tag-name ${TAG:-k8s-cluster}
